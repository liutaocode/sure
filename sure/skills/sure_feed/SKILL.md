...
- **Research narrows broad provider tags.** Provider taxonomies such as HuggingFace `audio-text-to-text` are broad evidence, not final SURE tasks. Before finalizing `task_type`, read model id, tags, README/model card, examples, and output contract. Apply narrowing precedence `sa_asr > sd > asr > speech_understanding`: `transcribe/transcription/asr` plus `diarization/speaker attribution` means `sa_asr`, not generic `speech_understanding`. Classify standalone Silero/FSMN speech-activity models as `vad`; an ASR, KWS, or SD model that merely mentions an internal VAD frontend keeps its primary task.
...
- **Allowed**: `model_input.task_type ∈ {asr,s2tt,sd,ser,se,vad,tts,vc,kws,slu,gr,speech_understanding,sa-asr,sa_asr}`; `deployment_type ∈ {local,api}`; `weights.source ∈ {huggingface,modelscope,local,api,pip,release_or_pypi}`.

...
| `task_type` | Required without `model_input_path` | `asr \| s2tt \| sd \| ser \| se \| vad \| tts \| vc \| kws \| slu \| gr \| speech_understanding \| sa-asr \| sa_asr`. SE aliases normalize to `se`; `voice-activity-detection` and `speech-activity-detection` normalize to `vad`. |
...
- **prepare_fixture**: Output = `fixture_manifest.json` {model_id, model_name, model_dir, task_type, source_dir, staged_dir, gt_jsonl, samples, sample_count, link_policy}. Use `scripts/prepare_fixture.py --run-dir <run_dir> --produces <run_dir>/artifacts/fixture_manifest.json` unless a custom source needs `--source-dir`. The helper selects the fixture source from `spec_validation.checks.fixture_availability.fixture_path` or `fixtures/tasks/<task>/...`, then copies it under `sure/models/<model_name>/fixture/<task>/<fixture_name>/` (`link_policy=copy`). `gt.jsonl` rows must reference relative audio paths inside the fixture directory and carry task annotations; VAD requires `speech_segments`, `sa_asr` requires speaker-attributed `segments`, and SE requires distinct noisy `audio` and clean `reference_audio` files; sample_count must be 1-5. For VAD/SD/SA-ASR, the source tree must contain no symlinks, staging copies only `gt.jsonl` and referenced PCM WAV files, and model input contains only `audio_path` plus approved protocol arguments. Reference segments, transcript text, and speaker-count annotations are never inference arguments.
...
- **generate_wrapper**: Output = `wrapper_manifest.json` {wrapper_path, model_py, server_py, ...}. The wrapper set lands in `sure/models/<model_name>/` (model.py, server.py, __init__.py, validate.py). Generated `validate.py` must preserve the template CLI and artifacts. VAD uses `detect_speech(audio_path, **protocol_args)` and returns required `speech_segments` plus optional `frame_scores`; SD uses `diarize`, and SA-ASR uses `transcribe_with_speakers`. Structured outputs are closed; raw/debug/reference/path fields and absolute paths or URIs are rejected. VAD times are finite, ordered, non-overlapping and bounded by the actual PCM WAV; supplied `frame_scores` continuously cover the full timebase. Templates live in `scripts/templates/`.
...
- **validate_import/load/infer/contract**: Output = `{*_passed, error, run_command|validate_py, log_path, ...}`. The gate executes `run_command` or `validate_py`; a boolean alone is not accepted. `validate_infer` is additionally Hook-guarded by a 1-5 row model-local fixture and must leave non-empty `sample_output.json`. VAD/SD/SA-ASR also require path-free, reference-free `sample_outputs.jsonl` with every fixture key in order and timing checked against the actual PCM WAV. Empty VAD/SD segments pass only for byte-level pure silence; SA-ASR segments and text are always non-empty. `validate_contract` re-reads every structured row.
...
- **save_artifacts**: Output = `artifact_manifest.json` {model_dir, artifacts.{required,conditional,optional}}. Gate checks model-local files exist and stages the selected delivery evidence. VAD/SD/SA-ASR make both `sample_output.json` and the path-free, reference-free `sample_outputs.jsonl` required bundle evidence. Local `package=none` requires the exact generated `model_runtime_manifest.json`; it does not infer one from `.venv`.
...
│   ├── validation.log
│   ├── sample_output.json
│   ├── sample_outputs.jsonl                    # all structured-task smoke rows

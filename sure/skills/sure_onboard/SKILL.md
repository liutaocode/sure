...
- **generate_wrapper**: Output = `wrapper_manifest.json` {wrapper_path, model_py, server_py, ...}. The wrapper set lands in `sure/models/<model_name>/` (model.py, server.py, __init__.py, validate.py). Generated `validate.py` must preserve the template CLI: `--stage import|load|infer|contract|all`, write `artifacts/<stage>_result.json`, write `artifacts/sample_output.json` during infer, and validate contract from `io_contract`. SD uses `diarize(audio_path, **protocol_args)`; SA-ASR uses `transcribe_with_speakers(audio_path, **protocol_args)`. Their output is a closed `segments` object with optional validated `num_speakers`; generic `predict`, raw/debug/reference/path fields, and absolute paths or URIs are rejected. Templates live in `scripts/templates/`. `config.yaml` may enable `protocols.strict_core` only when every conservative parameter (`temperature`, `do_sample`, `num_beams`, `num_return_sequences`, `seed`) maps to a property declared by the selected MCP tool `input_schema`, or is explicitly marked `model_param: null`, `status: not_applicable` with an architecture-specific reason. Omit/disable `strict_core` when that proof is unavailable; `/sure_eval` will still use `standard_system` by default.
...
- **validate_import/load/infer/contract**: Output = `{*_passed, error, run_command|validate_py, log_path, ...}`. The gate executes `run_command` or `validate_py`; a boolean alone is not accepted. `validate_infer` is additionally Hook-guarded: `fixture_manifest.json` must exist, point to `model_dir/fixture/<task>/.../gt.jsonl`, and declare 1-5 samples before inference can run. `validate_infer` must also leave a non-empty `sample_output.json` under the run or model artifacts directory. SD/SA-ASR additionally require `sample_outputs.jsonl` with every fixture key in order; each segment is checked for finite bounded times and task fields against the actual PCM WAV. Empty SD segments pass only for byte-level pure silence; SA-ASR segments and text are always non-empty. `validate_contract` re-reads all structured rows and the first-sample compatibility file, not only the first prediction.
...
- **save_artifacts**: Output = `artifact_manifest.json` {model_dir, artifacts.{required,conditional,optional}}. Gate checks model-local files exist and stages the selected delivery evidence. SD/SA-ASR make both `sample_output.json` and the path-free, reference-free `sample_outputs.jsonl` required bundle evidence. Local `package=none` requires the exact generated `model_runtime_manifest.json`; it does not infer one from `.venv`.
...
│   ├── validation.log
│   ├── sample_output.json
│   ├── sample_outputs.jsonl                    # all SD/SA-ASR smoke rows
